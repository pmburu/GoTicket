...
class Attendance(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    event = models.ForeignKey(Event, on_delete=models.CASCADE, related_name='my_event')
    attendee = models.ForeignKey(User, on_delete=models.PROTECT, related_name='attending')
    date_created = models.DateTimeField(auto_now_add=True)
    date_updated = models.DateTimeField(auto_now=True)
...
class Comments(models.Model):
    ...
    user = models.ForeignKey(User,on_delete=models.PROTECT, related_name='attendee_comments')
    event = models.ForeignKey('tickets.Ticket', on_delete=models.CASCADE, related_name='event_comments')
    date_created = models.DateTimeField(auto_now_add=True)
    date_updated = models.DateTimeField(auto_now=True)
